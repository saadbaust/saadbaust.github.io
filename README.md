...
> **Favicon.** `index.html` uses a small image file, `images/logo.png`, as the favicon. `gallery.html` uses an inline SVG data URI (`SA` initials in a blue rounded box), so it needs no image file. If you want both pages to match, point them at the same `href`. The old `s-logo.png` is no longer referenced anywhere — you can delete it if it's still in `images/`.
...
- **Profile & Intro** — name, title line, institution, research-focus line, the "About me" paragraph, and your CV link.
...
- **Section Visibility** — a switch for each home-page section (About, Experience, Research Projects, Journal Articles, Conference Proceedings, Teaching, Education, Skills, Certifications, Contact). Turn one off to hide that section from the live site; your content stays saved, it's just hidden, and the matching sidebar link disappears too.

Edit anything, click **Save**, done.

> **Profile photo.** Your profile picture (`images/saadahmed.png`) and the favicon (`images/logo.png`) are static files referenced directly by the page. To replace one, upload a file with the same name so it overwrites cleanly (deleting the old file first avoids duplicates piling up in `images/`).
...
- **`sitemap.xml`** — lists the home page plus an image sitemap (`image:image` entry) for the profile photo, so Google Image Search has an explicit list of crawlable images tied to that URL.
...
1. **Replace the profile image** in `images/` — your own `saadahmed.png` — and the favicon `images/logo.png`. (`gallery.html` uses an inline SVG favicon instead; edit the `SA` initials in its `<link rel="icon" …>` data URI, or point it at your own `logo.png` to match `index.html`.)
