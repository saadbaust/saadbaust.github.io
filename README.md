...
| `publications.bib` | All publications in BibTeX. The page reads it directly, splits it into entries, separates journals / conferences / preprints (`@misc`) / datasets (`@dataset`). Managed via the CMS "Publications" screen. |
...
One editor holding your entire `publications.bib`. **Paste the complete contents of your `.bib` file** — all entries together, exactly like opening the file in a text editor. Each entry must start with `@` (e.g. `@article{...}` for journals, `@INPROCEEDINGS{...}` for conferences, `@misc{...}` for preprints, `@dataset{...}` for datasets). The page splits the file into four lists — **Journal Articles**, **Conference Papers**, **Preprints**, and **Datasets** — each sorted newest first. A preprint is a `@misc` entry carrying `eprint` / `archivePrefix` / `primaryClass` fields (or an arXiv URL), which arXiv's own BibTeX already includes, so paste it unchanged. Datasets use the `@dataset` type; older non-preprint `@misc` dataset entries still render too, for backward compatibility. The Preprints and Datasets headings only appear when at least one such entry exists. To get an entry, click **Cite → BibTeX** on Google Scholar / IEEE Xplore / arXiv / Kaggle and paste it in. To remove an entry, delete its block.
